import { LowerCase } from "./name";

export const HOME = "/";
export const DOMAIN = `${LowerCase.FIRST}${LowerCase.LAST}.dev`;
export const SITE = `https://${DOMAIN}`;
export const EMAIL = `${LowerCase.FIRST}@${DOMAIN}`;
export const MAIL_TO = `mailto:${EMAIL}`;
export const GIT_HUB = "https://github.com/dmbdesignpdx";
export const LINKED_IN = "https://linkedin.com/in/danielblakepdx";
export const CODE_PEN = "https://codepen.io/dmbdesignpdx";
